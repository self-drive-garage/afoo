...
#include "afoo/io/io_interface.hpp"
...
/// This class encapsulates all the functionalities required to open, close,
/// read from, and write to a serial port. It uses Boost.Asio's asynchronous I/O
/// operations to manage serial port data transmission and reception.
class BoostSerialDevice : public IOInterface<std::string> {
 public:
  ...
  /// @return A tuple containing a Status object and an optional string with the
  /// received data.
  ...
  bool isOpened() const override { return isOpened_; };
 ...
 private:
  ...
  /// This private method is used to encapsulate the low-level write
  /// functionalities.
  /// @param message The message to be written to the serial port.
  /// @return Status indicating success or failure of the internal write
  /// operation.
  Status innerWrite(const std::string& message);

  /// Validates the response against the request.
  /// Checks if the response received from the device matches the sent request,
  ...
  boost::asio::io_context
      ioContext_;  ///< Boost.Asio IO context for managing I/O services.
  boost::asio::serial_port serialPort_;  ///< Boost.Asio serial port object.
  bool isRequestEchoed_;  ///< Flag indicating whether the device echoes back
                          ///< the sent requests.
  boost::asio::streambuf
      response_;  ///< Buffer for storing responses from the serial port.

  rclcpp::Logger logger_ = rclcpp::get_logger("BoostSerialDevice");
};
