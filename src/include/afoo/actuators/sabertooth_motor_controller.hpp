...
/// @brief Class for controlling a Sabertooth motor controller via packetized
/// serial.
class SabertoothMotorController {
 ...
 private:
  std::string
      port_;  ///< The serial port connected to the Sabertooth controller.
  uint32_t baudRate_;  ///< Baud rate for serial communication.
  io::BoostSerialDeviceManager
      manager_;  ///< Manages serial device communication.
