...
io::Status SabertoothMotorController::mixedModeDrive(int speed, int turn) {
  ...
  int speedValue =
      std::abs(speed);  // No need to scale if already in the range 0-127

  // Determine the correct command for turning and scale the absolute turn value
  // to 0-127
  int turnCommand = (turn >= 0) ? 10 : 11;
  int turnValue =
      std::abs(turn);  // No need to scale if already in the range 0-127
...
char SabertoothMotorController::calculateChecksum(int address, int command,
                                                  int data) {
